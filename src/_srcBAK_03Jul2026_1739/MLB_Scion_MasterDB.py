...
class scionMASTERDB:
    ...
    def _loadMASTERDB(self):
        try:
            ...
            #self._masterdb_df = pd.read_csv(self.masterdbfname, header=0, infer_datetime_format=True)
    ...
    def copyMasterStructure(self, _includeBlankRow=True):
        ...
        try:
            #a. copy one row
            #_master_copy_df = pd.DataFrame(data=None, columns=self._masterdb_df.columns, index=self._masterdb_df.index)
            ...
            #_master_copy_df = _master_copy_df.replace('None', np.nan) #replace None with np.nan first
            #_master_copy_df = _master_copy_df.replace(_master_copy_df, MLB_dbvar.NO_DATA)
            ...
            print("\nscionMASTERDB.copyMasterStructure(): Unexpected error encountered when copying the structure of the master database!")
            raise
