...
class scionCFG:
    ...
    # -- V26.06b: generic ensemble-aware config accessors --
    # ENS1 (StatsOnly/V127) reads the base getSysProb* values; ENS2
    # (LeanStatsOnly/V129) reads getSysProbLEANSTATSONLY*. Collapses per-ensemble
    # branching at call sites into one ens-parameterised call.
    def getEnsProbAgreeThresh(self, ens):
        return self.getSysProbAgreeThresh() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYAgreeThresh()

    def getEnsProbPointsGap(self, ens):
        return self.getSysProbPointsGap() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYPointsGap()

    def getEnsProbBPVarDP(self, ens):
        return self.getSysProbBPVarDP() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYBPVarDP()

    def getEnsProbThresh(self, ens):
        return self.getSysProbThresh() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYThresh()

    def getEnsProbBookieMin(self, ens):
        return self.getSysProbBookieMin() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYBookieMin()

    def getEnsProbBookieMax(self, ens):
        return self.getSysProbBookieMax() if ens == MLB_global.MODEL_PROBENS1 else self.getSysProbLEANSTATSONLYBookieMax()
